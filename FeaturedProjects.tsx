...
import { featuredProjectsConfig, products } from '../config';
...
export function FeaturedProjects() {
  ...
  return (
    <section
      ...
      style={{ background: 'linear-gradient(180deg, #1a3d2a 0%, #0d1f15 100%)' }}
    >
      {/* Gold accent glow */}
      <div 
        className="absolute top-1/3 left-0 w-[500px] h-[500px] opacity-10"
        style={{ 
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        ...
        <div ref={headerRef} className="flex flex-col md:flex-row md:items-end md:justify-between mb-16 md:mb-20 opacity-0">
          ...
          {featuredProjectsConfig.viewAllText && (
            <Link
              to={featuredProjectsConfig.viewAllHref || '/category/shoes'}
              className="mt-6 md:mt-0 inline-flex items-center gap-2 font-body text-sm transition-colors duration-300 group"
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  <div className="absolute inset-0 pointer-events-none">
                    ...
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                      ...
                      <div className="w-6 h-px bg-[#d4af37]/50 -mt-3 -ml-[11px]" />
                    </div>
                  </div>
                  ...
                  <div 
                    ...
                    style={{ background: 'linear-gradient(to top, rgba(13, 31, 21, 0.7), transparent)' }}
                  />

                  {/* Quick order button */}
                  <div className="absolute bottom-4 left-4 right-4 opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300">
                    <span className="btn-luxury w-full flex items-center justify-center gap-2 text-sm py-3">
                ...
                <div className={`project-content opacity-0 ${index % 2 === 1 ? 'md:order-1 md:text-right' : ''}`}>
                  ...
                  <h3 
                    className="project-text-item text-2xl md:text-3xl lg:text-4xl font-sans font-bold tracking-tight mb-4"
                    style={{ color: '#d4af37' }}
                  >
                    {project.title}
                  </h3>
                  <p 
                    className="project-text-item font-body text-base md:text-lg leading-relaxed mb-6"
                  ...
                  {featuredProjectsConfig.viewProjectText && (
                    <Link
                      to={`/product/${product?.id || '#'}`}
                      className={`project-text-item inline-flex items-center gap-2 font-body text-sm border-b pb-1 transition-colors duration-300 group/link ${
                        index % 2 === 1 ? 'md:flex-row-reverse' : ''
                      }`}
                      style={{ 
                        color: '#d4af37', 
